...
gftrl_op_module = tf.load_op_library(
    resource_loader.get_path_to_datafile('libgftrl_op.so'))


@tf_export(v1=["train.GFtrlOptimizer"])
class GFtrlOptimizer(optimizer.Optimizer):
  ...
  def __init__(self,
    ...
    """
    super(GFtrlOptimizer, self).__init__(use_locking, name)
  ...
  def __apply_dense(self, grad, var):
    accum = self.get_slot(var, "accum")
    linear = self.get_slot(var, "linear")
    if self._l2_shrinkage_regularization_strength <= 0.0:
      return gftrl_op_module.apply_g_ftrl(
          var,
          accum,
          linear,
          grad,
  ...
  def __resource_apply_dense(self, grad, var):
    accum = self.get_slot(var, "accum")
    linear = self.get_slot(var, "linear")
    if self._l2_shrinkage_regularization_strength <= 0.0:
      return gftrl_op_module.resource_apply_g_ftrl(
          var.handle,
          accum.handle,
          linear.handle,
          grad,
  ...
  def _apply_sparse(self, grad, var):
    ...
    linear = self.get_slot(var, "linear")
    if self._l2_shrinkage_regularization_strength <= 0.0:
      return gftrl_op_module.sparse_apply_g_ftrl(
          var,
          accum,
          linear,
          grad.values,
  ...
  def _resource_apply_sparse(self, grad, var, indices):
    ...
    linear = self.get_slot(var, "linear")
    if self._l2_shrinkage_regularization_strength <= 0.0:
      return gftrl_op_module.resource_sparse_apply_g_ftrl(
          var.handle,
          accum.handle,
          linear.handle,
          grad,
