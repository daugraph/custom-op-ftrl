...
def define_wide_deep_flags():
  ...
  flags.DEFINE_string(name='timeline_dir',  default='/tmp/', help='Training timeline dir')
  flags.DEFINE_integer(name='max_step',  default=10000, help='Max training step')
...
def run_loop(name, train_input_fn, eval_input_fn, model_column_fn,
  ...
  phook = tf.train.ProfilerHook(save_steps=1000, output_dir=flags_obj.timeline_dir)
  # train_hooks.append(phook)
  train_spec = tf.estimator.TrainSpec(input_fn=train_input_fn, max_steps=flags_obj.max_step, hooks = [phook])
  ...
  for n in range(flags_obj.train_epochs // flags_obj.epochs_between_evals):
    ...
    tf.logging.info('After at epoch %d / %d',
